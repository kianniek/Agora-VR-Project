...
public class AsyncGPUReadbackMeshMusic : MonoBehaviour
{
    public float raiseAmount;
    public Transform reverencePosition;
    ...
    private void Start()
    {
        ...
        //compute shader
        _kernel = computeShader.FindKernel("CSMainMusic");
        uint threadX = 0;
        uint threadY = 0;
        uint threadZ = 0;
        ...
        oBuffer = new ComputeBuffer(mesh.vertexCount, 12); // 3*4bytes = sizeof(Vector3)
        if (vertData.IsCreated)
        {
            cBuffer.SetData(vertData);
            oBuffer.SetData(vertData);
        }
        computeShader.SetBuffer(_kernel, "vertexBuffer", cBuffer);
        computeShader.SetBuffer(_kernel, "originalVertexBuffer", oBuffer);
        Matrix4x4 modelToWorldMatrix = mf.transform.localToWorldMatrix;
        computeShader.SetMatrix("UNITY_MATRIX_MVP", modelToWorldMatrix);
    ...
    void Update()
    {
        ...
        computeShader.SetFloat("_RaiseAmount", raiseAmount);
        float[] reverencePositionToFloatArray = { reverencePosition.position.x, reverencePosition.position.y, reverencePosition.position.z };
        computeShader.SetFloats("_ReverencePosition", reverencePositionToFloatArray);
