        ...
        */
        ...
        private void StructureNodeConstructor (StructureNode.NodeType nodeType, NodeOrientation nodeOrientation) {
            ...
            if (addUpstreamChildrenPort) {
                ...
                upstreamChildrenPort = StructurePort.Create<Edge> (portOrientation, Direction.Input, Port.Capacity.Multi, typeof(StructureNode));
                upstreamChildrenPort.name = upChildrenPortName;
                inputContainer.Add (upstreamChildrenPort);
            }
            if (addDownstreamChildrenPort) {
                downstreamChildrenPort = StructurePort.Create<Edge> (portOrientation, Direction.Output, Port.Capacity.Multi, typeof(StructureNode));
                downstreamChildrenPort.name = downChildrenPortName;
                outputContainer.Add (downstreamChildrenPort);
            }
            if (addUpstreamParentPort) {
                upstreamParentPort = StructurePort.Create<Edge> (portOrientation, Direction.Output, Port.Capacity.Single, typeof(StructureNode));
                upstreamParentPort.name = upParentPortName;
                outputContainer.Add (upstreamParentPort);
            }
            if (addDownstreamParentPort) {
                downstreamParentPort = StructurePort.Create<Edge> (portOrientation, Direction.Input, Port.Capacity.Single, typeof(StructureNode));
