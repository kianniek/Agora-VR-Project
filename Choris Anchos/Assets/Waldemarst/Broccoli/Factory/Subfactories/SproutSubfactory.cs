    ...
    public class SproutSubfactory {
        ...
        public void ProcessSnapshot (int snapshotIndex, bool clearCompositeManager = true, MaterialMode materialMode = MaterialMode.Composite, bool isNewSeed = false) {
            ...
            */
            ...
            ProcessSnapshotPipelineMesh (clearCompositeManager, materialMode, isNewSeed);
        ...
        /// <param name="materialMode">Materials mode to apply.</param>
        /// <param name="isNewSeed"><c>True</c> to create a new preview (new seed).</param>
        public void ProcessSnapshotPipelineMesh (bool clearCompositeManager = true, MaterialMode materialMode = MaterialMode.Composite, bool isNewSeed = false) {
            if (branchDescriptorIndex < 0 || branchDescriptorCollection.branchDescriptors.Count == 0) return;
        ...
        /// </summary>
        /// <param name="branchDescriptor">Branch Descriptor to process as snapshot.</param>
        public void ProcessSnapshotPolygons (BranchDescriptor branchDescriptor) {
            ...
            // Generate curve.
            GenerateSnapshotCurve (branchDescriptor);
        ...
        /// <summary>
        /// Creates the curve as axis for the snapshot of a branch descriptor.
        /// </summary>
        /// <param name="branchDescriptor">Branch Descriptor to create the curve to.</param>
        public void GenerateSnapshotCurve (BranchDescriptor branchDescriptor) {
            // Creal existing snapshot curve.
            branchDescriptor.curve.RemoveAllNodes ();
            ...
            while (currentBranch != null) {
                // Add Current Branch nodes.
                for (int i = (isFirstBranch?0:1); i < currentBranch.curve.nodeCount; i++) {
                    node = currentBranch.curve.nodes [i].Clone ();
                    node.position += offset;
                    branchDescriptor.curve.AddNode (node, false);
                }
                isFirstBranch = false;
                offset = node.position;
                if (currentBranch.followUp == null) {
                    // Add final sprout.
                    float sproutPosition = 0f;
                    BroccoTree.Sprout lastSprout = null;
                    for (int i = 0; i < currentBranch.sprouts.Count; i++) {
                        if (currentBranch.sprouts [i].position > sproutPosition) {
                            sproutPosition = currentBranch.sprouts [i].position;
                            lastSprout = currentBranch.sprouts [i];
                        }
                    }
                    if (sproutPosition == 1f) {
                        Vector3 lastNodePos = currentBranch.GetPointAtPosition (1f)
                            + (lastSprout.sproutDirection * lastSprout.meshHeight);
                        BezierNode lastNode = new BezierNode (lastNodePos);
                        branchDescriptor.curve.AddNode (lastNode, false);
                    }
                }
                currentBranch = currentBranch.followUp;
            }

            branchDescriptor.curve.Process ();
        }
        /// <summary>
            ...
            string texturePath = "") 
        {
            if (snapshotIndex >= branchDescriptorCollection.branchDescriptors.Count) {
                ...
                branchDescriptorIndex = snapshotIndex;
                ProcessSnapshotPipelineMesh (true, materialMode);
            ...
            string compositePath) 
        {
            ...
            if (branchDescriptorCollection.branchDescriptors.Count == 0) {
                ...
                for (int i = 0; i < branchDescriptorCollection.branchDescriptors.Count; i++) {
                    ...
                    branchDescriptorIndex = i;
                    BranchDescriptorCollectionToPipeline ();
                    ProcessSnapshotPipelineMesh (false);
